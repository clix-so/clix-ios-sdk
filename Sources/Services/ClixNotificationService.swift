...
public class ClixNotificationService: UNNotificationServiceExtension {
            ...
            if let mediaUrl = request.content.userInfo["media_url"] as? String,
                ...
                Task {
                    if let attachment = try? await downloadAndAttachMedia(
                        url: url, type: request.content.userInfo["media_type"] as? String ?? "image"
                    ) {
        ...
        try await withCheckedThrowingContinuation { continuation in
            let task = URLSession.shared.downloadTask(with: url) {
                ...
                guard let temporaryFileLocation = temporaryFileLocation,
                    ...
                    continuation.resume(
                        throwing: NSError(
                            domain: "ClixNotificationService", code: -1,
