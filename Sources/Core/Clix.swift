...
public class Clix {
    ...
    public func initialize(apiKey: String, endpoint: String, withConfig config: ClixConfig?)
        async throws {
    ...
    func handleNotificationResponse(_ response: UNNotificationResponse) async throws {
        try await trackEvent(
            "push_opened", properties: ["payload": response.notification.request.content.userInfo])
    }
