...
public class ClixNotification: NSObject, UNUserNotificationCenterDelegate, MessagingDelegate {
  ...
  public func userNotificationCenter(
    ...
    // Use custom handler if available, otherwise use default behavior
    if let handler = willShowHandler {
      let options = handler(notification)
      completionHandler(options)
      return
    }

    // Default behavior with image handling
  ...
  public func userNotificationCenter(
    ...
    if let handler = openedHandler {
      handler(userInfo)
    }

    // Always call internal handler for Clix analytics
  ...
  public func handleSilentNotification(
