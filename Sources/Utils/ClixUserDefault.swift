...
public class ClixUserDefault {
  ...
  private enum Keys {
    ...
    static let deviceId = "clix_device_id"
  ...
  public func configure(appGroupId: String) {
    if let sharedDefaults = UserDefaults(suiteName: appGroupId) {
      ...
      let deviceId = sharedDefaults.string(forKey: Keys.deviceId) ?? "<not set>"
      ...
      NSLog(
        "[ClixUserDefault] Current values - projectId: \(projectId), apiKey: \(apiKey), endpoint: \(endpoint), deviceId: \(deviceId)"
      )
    } else {
  ...
  public func saveConfig(_ config: ClixConfig, deviceId: String) {
    ...
    NSLog(
      "[ClixUserDefault] Saving config - projectId: \(config.projectId), deviceId: \(deviceId), apiKey: \(config.apiKey), endpoint: \(config.endpoint)"
    )
    ...
    let savedDeviceId = userDefaults.string(forKey: Keys.deviceId) ?? "<not set>"
    NSLog(
      "[ClixUserDefault] Verified saved values - projectId: \(savedProjectId), apiKey: \(savedApiKey), endpoint: \(savedEndpoint)"
    )
  }
  ...
  public func getEndpoint() -> String {
    return userDefaults?.string(forKey: Keys.endpoint) ?? "https://external-api.clix.so"
  }
